...
var throwCombination = [1, 1, 1, 3, 3];
var numberOfDice = 5;
var throwScore = 0;
var roundScore = 0;
    ...
    for (var i = 0; i < dieNum.length; i++) {
      ...
      if (frequence === 5) {
        ...
        if (countRecurrence[dieNum[i - 1 % dieNum.length]] === 2) {
          if (number === 1) {
            throwScore += brelant(Number(dieNum[i])) + 50 * 10;
          } else {
            throwScore +=
              brelant(Number(dieNum[i])) +
              50 * Number(dieNum[i - 1 % dieNum.length]);
          }
          ...
          if (number === 1) {
            ...
            throwScore += brelant(number); // cas ou il y a un 1 ou un 5 en plus du brelan.
          }
        }
      } else if (frequence <= 2) {
        if (number === 1) {
